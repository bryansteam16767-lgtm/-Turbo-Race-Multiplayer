  ...
  io.on("connection", (socket) => {
    ...
    socket.on("joinRoom", ({ roomId }) => {
      if (rooms[roomId]) {
        ...
        const gameInProgress = room.status === 'racing';
        
        // If game is in progress, join as spectator (don't add to players list for physics, just for tracking if needed)
        // Actually, for simplicity, we can add them to players but mark them as spectator in the client.
        // But if we add them to room.players, they might appear in the leaderboard.
        
        if (gameInProgress) {
            // Join as spectator
            socketRoomMap[socket.id] = roomId;
            socket.join(roomId);
            socket.emit("roomJoined", { roomId, players: room.players, isHost: false, gameInProgress: true });
            return;
        }
        ...
        // Notify the joiner
        socket.emit("roomJoined", { roomId, players: room.players, isHost: false, gameInProgress: false });
    ...
    socket.on("getRooms", () => {
        // Return active rooms with player counts
